...
| CLI 옵션 | 의미 | 기본값 |
|---|---|---|
| `--box-lookback` | 박스를 만드는 직전 봉 수 | 24 |
| `--box-width` | 박스폭 상한 `(상단-하단)/하단` | 0.04 |
| `--vol-mult` | 돌파봉 거래량 / 평균거래량 하한(거짓돌파 필터) | 1.5 |
| `--buffer` | 박스 경계 너머 진입 버퍼 | 0.001 |
| `--atr-period` | ATR 기간 | 14 |
| `--atr-trail` | 트레일링 스탑 ATR 배수 (클수록 추세 길게 홀딩) | 3.0 |
...
1. **실데이터 백테스트** — 여러 심볼·기간으로 파라미터 견고성 검증 (과최적화 주의).
2. **워크포워드 분석** — 구간을 나눠 인샘플 최적화 → 아웃샘플 검증.
